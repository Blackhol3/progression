...
export class ProgramService {
	...
	constructor() {
		for (let knowledge of DataKnowledge) {
			...
			if (storedValue != null) {
				this.knowledges.last().sequences = JSON.parse(storedValue);
			}
		}
		...
		for (let knowHow of DataKnowHow) {
			...
			if (storedValue != null) {
				this.knowHows.last().sequences = JSON.parse(storedValue);
			}
		}
			...
			for (let descendantKnowledge of descendantKnowledges)
			{
				...
				for (let competence of competences)
				{
					for (let knowHow of competence.knowHows) {
						subSequences.push(...knowHow.sequences);
					}
				}
	...
	updateKnowledgeSubSequences(knowledge: Knowledge): void {
		...
		for (let knowHow of knowHows)
		{
			...
			for (let finalAssociatedKnowledge of finalAssociatedKnowledges) {
				subSequences.push(...finalAssociatedKnowledge.sequences);
			}
